...
```
Folders for Claude has a single purpose: letting users organize their claude.ai conversations into folders shown inside Claude's sidebar, so they can file, search and reopen chats quickly.
```
...
**Host permission / content script: https://claude.ai/***
```
The content script inserts the Folders section into Claude's sidebar and reads the ID (from the link URL) and visible title of chats so they can be added to folders and kept up to date when renamed. It never reads message content.
```
